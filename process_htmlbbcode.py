...
# #Update By yangshou/sjchenkan (2020-1-16)
...
def run():
	...
	#开始转码
        resp = trans(message)
        resp = resp.replace("/file.php?f=","/data/upload/")
        resp = resp.replace("[img]/data/","[img]"+srv+"/data/")
        resp = resp.replace("[img]/data/","[img]"+srv+"/data/")
        resp = resp.replace("&t=jpg","")
        resp = resp.replace("&t=png","")
        #print(resp)
        #return
        resp = resp.replace("'", "\\'")
        resp = resp.replace('\\', '\\\\')
